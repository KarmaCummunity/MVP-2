...
// `Share.share({ url: <fileUri> })` can attach the binary directly.
// Native-only: web uses Blob/File via fetch in `sharePost.ts`. Returns
// `null` (not throws) on any failure so the caller can fall back to the
// URL-only share path (`shared via OG card`) instead of dropping the
// affordance entirely.
...
function extFromUrl(url: string): string {
  ...
  // Allow-list of image extensions we may have uploaded (R-MVP-Items-3).
...
export async function downloadPostImageForShare(
  ...
  try {
    // Lazy-require so vitest (node env, no expo native module) doesn't choke
    // when sharePost.ts is exercised through web-flow tests.
