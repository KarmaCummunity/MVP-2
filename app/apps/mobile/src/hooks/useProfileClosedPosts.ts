...
import type { ProfileClosedPostsListMode } from '@kc/domain';
...
export interface UseProfileClosedPostsArgs {
  ...
  enabled?: boolean;
  /** Default `standard` (main Closed tab). `owner_only_me` for Hidden screen. */
...
export function useProfileClosedPosts({
  ...
  const query = useInfiniteQuery({
    queryKey: ['profile-closed-posts', profileUserId, viewerUserId, listMode],
    queryFn: ({ pageParam }) =>
