...
## Branded share URL via `/p/:id`

To keep the user-visible URL on the app's own domain instead of
`<ref>.supabase.co/functions/v1/share-post/…`, the web bundle ships a
`serve.json` (generated by `app/scripts/web-postbuild.mjs`) with a 302
redirect:

```
/p/:id  →  ${EXPO_PUBLIC_SUPABASE_URL}/functions/v1/share-post/:id
```

Setting `EXPO_PUBLIC_SHARE_BASE_URL=https://karma-community-kc.com/p` in
the mobile env then makes the share message read `…/p/<id>`. Crawlers
follow the 302 to this function and read its OG card normally; humans
get one extra 302 (this function then bounces them to
`karma-community-kc.com/post/<id>`).
