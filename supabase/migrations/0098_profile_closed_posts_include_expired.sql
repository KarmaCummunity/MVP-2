-- 0098_profile_closed_posts_include_expired | TD-70 — FR-POST-013 AC3
-- Extends profile_closed_posts (0088) to surface owner-only expired posts in
-- the Closed tab. The Republish CTA on the FE acts on rows returned here.
--
-- Semantics:
--   - Expired rows appear only when viewer = profile owner (`p.owner_id = p_viewer_user_id`).
--   - No recipient-role surfacing for expired (the post was never closed/delivered).
--   - closed_at uses `posts.updated_at`, which the 0097 cron refreshes via set_updated_at.
--   - 'owner_only_me' list mode keeps showing only OnlyMe visibility posts (no change).
