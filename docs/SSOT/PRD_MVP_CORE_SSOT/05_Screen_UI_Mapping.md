...
ה-MVP מורכב מ-**27 מסכים ומודלים** בלבד (לעומת 100+ ב-V2; יישור P1 #13). כל מסך נדרש להוכחת PMF. הספירה כוללת 7 מסכי Auth, 4 מסכי ליבה, 4 מסכי פרופיל, 2 מסכי צ'אט, 4 מסכי הגדרות, ו-6 מודלים/מסכים מיוחדים.
...
> **בסך הכל: 27 מסכים/מודלים** (יישור 5.1 לעיל). לעומת ~100 מסכים ב-V2 → צמצום של ~73%. זה הוא המהות של MVP.
...
* **רכיבים:**
    * **חשבון:** שם, מייל/טלפון, **שינוי סיסמה** (יישור P2 #28: שורה זו **מוצגת רק למשתמשים שנרשמו עם מייל+סיסמה**. למשתמשי Google/Apple/OTP טלפון השורה לא מוצגת כלל). ראה 03 §3.1.3a.
