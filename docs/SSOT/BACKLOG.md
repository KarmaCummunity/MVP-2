...
| P2.32 | **Dark mode — Appearance toggle (FR-SETTINGS-014)** — adds System / Light / Dark theme picker in Settings → Appearance with a warm-tinted dark palette (deep warm-brown surfaces, lifted orange primary). Ships theme primitives in `@kc/ui` (`ThemeProvider`, `useTheme`, `makeUseStyles`, `lightColors` / `darkColors`), AsyncStorage-persisted `themeStore`, and migrates the high-visibility screens (TabBar, TopBar, Screen wrapper, Settings + SettingsRow, Welcome/Auth, PostCard, Appearance screen). Native StatusBar style + web html background follow the active scheme. Remaining screens use the legacy `colors` export and render in light until migrated (TD-130). | agent-fe | 🟡 In progress | `spec/11_settings.md` FR-SETTINGS-014 |
