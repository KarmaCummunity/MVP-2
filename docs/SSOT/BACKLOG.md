...
| P2.26 | **Closed-post Hide bug fix** — `usePostActorPrivacyModel.onAudienceChange` fans out to `posts.visibility` for closed-post owners so the Hidden Closed lane routes correctly; `UpdatePostUseCase` accepts visibility-only patches on `closed_delivered` / `deleted_no_recipient`. | agent-fullstack | ✅ Done | `spec/04_posts.md` FR-POST-009 AC5; `spec/02_profile_and_privacy.md` FR-PROFILE-001 AC4 note; `DECISIONS.md` D-34 |
| P2.27 | **Saved screen open/closed split** — `/profile/saved` splits into "פוסטים פתוחים" and "פוסטים סגורים" sections (client-side partition); dead `self_saved` empty variant removed after migration. | agent-fe | ✅ Done | `spec/02_profile_and_privacy.md` FR-PROFILE-016 AC6 |
| P2.28 | **Admin-removed screen open/closed split** — `/profile/removed` splits by `posts.status_before_admin_removal` (D-35); migration `0096` adds nullable column, updates `admin_remove_post` to capture prior status atomically. Legacy NULL rows default to prior-open lane. | agent-fullstack | ✅ Done | `spec/04_posts.md` FR-POST-008 AC5; `spec/02_profile_and_privacy.md` FR-PROFILE-001 AC4 cross-ref; `DECISIONS.md` D-35; migration `0096_posts_status_before_admin_removal.sql` |
