...
| P2.33 | **Share post via link with image preview (FR-POST-023)** — header share button on post detail (Public + open + has image); platform share via `Share.share` (native) / `navigator.share` + clipboard fallback (web). Share icon sits in the trailing corner with ⋮ menu to its left. iOS attaches the first image binary alongside the OG link via `expo-file-system`; Web Share Level 2 attaches via `navigator.canShare({ files })`. New `share-post` Edge Function returns OG-card HTML for crawlers + 302 redirect for humans to `/post/<id>` universal link. AuthGate captures pending deep link before redirecting unauth users to `(auth)`, restores after sign-in. Web bundle emits `serve.json` redirecting `/p/:id` → Edge Function so `EXPO_PUBLIC_SHARE_BASE_URL=https://karma-community-kc.com/p` keeps the user-visible URL on the branded host; Android `Share.share` no longer passes `url` (RN appended it, surfacing the link twice in WhatsApp). | agent-fullstack | 🟡 In progress | `spec/04_posts.md` FR-POST-023 |
