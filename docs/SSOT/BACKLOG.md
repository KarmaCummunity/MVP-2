...
| P3.A1 | **Admin Portal A1 — Reports Dashboard.** Replaces the chat-based moderation flow with `/admin/reports` inbox + `/admin/reports/[caseId]` case detail. Closes TD-94 cascade-dismiss sub-item (per-case restore — no cascade). Introduces `EXPO_PUBLIC_ADMIN_PORTAL_REPORTS` feature flag and the chat-flow coexistence/deprecation path. | 🟡 In progress | `spec/12_super_admin.md` FR-ADMIN-012/013/014; plan `docs/superpowers/plans/2026-05-26-admin-portal-a1-reports.md` |
