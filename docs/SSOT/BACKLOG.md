...
| P2.33 | **Share post via link with image preview (FR-POST-023)** — header share button on post detail (Public + open + has image); platform share via `Share.share` (native) / `navigator.share` + clipboard fallback (web). New `share-post` Edge Function returns OG-card HTML for crawlers + 302 redirect for humans to `/post/<id>` universal link. AuthGate captures pending deep link before redirecting unauth users to `(auth)`, restores after sign-in. | agent-fullstack | 🟡 In progress | `spec/04_posts.md` FR-POST-023 |
