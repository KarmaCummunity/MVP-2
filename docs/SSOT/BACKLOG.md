...
| P2.24 | **In-app Google sign-in bottom sheet (web, FR-AUTH-002)** — replace `window.open` popup with a draggable 90% bottom sheet rendering the GIS button + FedCM picker; new `signInWithIdToken` port + Supabase adapter; native flows unchanged. Requires `EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID` (PM action). | agent-fe | 🟡 In progress | `spec/01_auth_and_onboarding.md` FR-AUTH-002 (implementation note 2026-05-17); `DECISIONS.md` D-33 |
