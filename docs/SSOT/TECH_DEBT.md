...
| TD-40 | 🟠 | `SupabaseUserRepository` is a slice stub. Wired: `getOnboardingState`, `setBasicInfo`, `setOnboardingState`, `setAvatar` (P0.3.b 2026-05-08). The remaining 18 `IUserRepository` methods throw `not_implemented`. Adapter file: `app/packages/infrastructure-supabase/src/users/SupabaseUserRepository.ts`. The `not_implemented` errors include the slice that owns each method. | P0.4 (`findByAuthIdentity`, `findById`) · P1.1 (follows + follow-requests) · P1.4 (blocks) · P2.4 (`update`, `findByHandle`, `delete`) |
