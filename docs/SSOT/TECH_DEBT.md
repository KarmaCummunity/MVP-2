...
| TD-52 | 🟠 | **`SupabaseSearchRepository` over 200-line cap (418 lines).** Landed on main in commit `e115b4c add search mechanism`. Split into per-entity adapter helpers (e.g. `searchPosts.ts`, `searchUsers.ts`) under `packages/infrastructure-supabase/src/search/`. Allowlisted at 418 — file cannot grow until split. | Opportunistic (FR-FEED-006..014 / P1.2) |
...
| TD-124 | 🟠 | **Search UI files over 200-line cap.** Landed on main in commit `e115b4c add search mechanism`. `apps/mobile/app/(tabs)/search.tsx` (646), `apps/mobile/src/components/SearchFilterSheet.tsx` (306), `apps/mobile/src/components/SearchResultCard.tsx` (354). Extract presentational subcomponents (`<SearchHeader>`, `<SearchResultsList>`, `<FilterChipRow>`, etc.) and split the screen into hooks + render-only pieces. All three allowlisted at current line counts. | Opportunistic (FR-FEED-006..014 / P1.2) |
