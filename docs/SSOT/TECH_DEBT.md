...
| **Last Updated** | 2026-05-11 (P1.1 — TD-14 + TD-40 closed (partial); TD-124..TD-128 opened; TD-129..TD-133 added from FR-ADMIN-009 + search-mechanism; TD-112 note updated.) |
...
| TD-125 | 🟠 | **Optimistic update missing on Follow button.** FR-FOLLOW-001 AC4 specifies "button updates to 'Following ✓' optimistically; rollback on server error." The current `/user/[handle]/index.tsx` awaits the network call before updating; on slow connections the button is unresponsive for the round-trip. Fix by setting query data via `qc.setQueryData` before the await, with rollback in catch. | Opportunistic / P1.1 polish |
| TD-126 | 🟠 | **Cooldown error toast lacks days-remaining.** When a `cooldown_active` FollowError surfaces from a manual retry, the alert says only "נסה שוב כשהקירור יסתיים". The error carries `cooldownUntil`; compute remaining days and show "ניתן לשלוח שוב בעוד N ימים" for parity with the disabled-button subtitle. | Opportunistic / P1.1 polish |
| TD-127 | 🟠 | **Report action absent from ⋮ menu on Other Profile.** FR-PROFILE-002 AC1 requires both Report and Block in the ⋮ menu. Block is wired; Report (FR-MOD-*) is P1.3 scope. When P1.3 ships, wire Report into the same ⋮ menu and move from "block-direct" to a proper menu UI. | P1.3 |
