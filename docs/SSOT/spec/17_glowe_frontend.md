...
**Realtime slice (2026-07-19, D-182):** Supabase Realtime delivery in an open thread (`glowe-chat-realtime.js` mirroring KC `SupabaseChatRealtime.ts`); live inbox sort/preview/unread and header badge while signed in; optimistic send with inline retry; day separators (Today / Yesterday / date); counterpart name links to `profile.html?id=…`. Pure helpers in `glowe-messages.js` + vitest coverage. No new migrations/RPCs. Plan: `docs/superpowers/plans/2026-07-19-glowe-chat-realtime.md`.
