...
- AC9. **Header nav menu on phone widths.** ✅ The static two-button "Log In" / "Join GloWe" markup (dead — already overridden at runtime by `normalizeHeaderAuthButtons()`, since Google-only auth makes sign-in and join the same action) was removed from every page's header HTML, leaving the single "Sign up / Sign in" button as the only source, no-JS fallback included. Below `900px` the label row (Home, Wishing Well, Organizations, Community, About) collapses behind a `.nav-toggle` menu icon in the top bar (`ensureNavToggle()` in `js/app.js`) instead of wrapping into extra rows or (below `680px`, where `.main-nav` was previously `display: none !important` in favor of the 4-item bottom tab bar) disappearing entirely — the panel now also restores reachability to the labels the bottom bar omits (Organizations, Forums, About) on phones.
