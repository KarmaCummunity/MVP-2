...
- AC4. The translatable-field registry excludes proper names: `glowe_post` = {`title`, `text`, `tags[]` via `tags.N`}; `glowe_comment` = {`text`}; `glowe_opportunity` = {`title`, `description`, `requirements`, `responsibilities`} (NOT `organization`); `glowe_wish` = {`title`, `text`}; `glowe_project` = {`title`, `description`}; `glowe_profile` = {`about`/`org_description`, `focus`, `needs`, `org_field`} (NOT `display_name`). No name field is ever submitted for translation. Person/org display names use bilingual columns (FR-GLOWE-024 / D-179) instead.
