...
## FR-TRANSLATE-005 — GLOWE web reader translation (🟡 In progress)

> **Status:** 🟡 In progress. Surfaces the translation engine into the GLOWE static web frontend (`app/apps/glowe-web`), which shares the KC Supabase backend. GLOWE is the active MVP (AGENTS.md § Active product focus); the KC app frontend is out of scope. Design + plan: `docs/superpowers/specs/2026-07-04-glowe-web-translation-design.md`, `docs/superpowers/plans/2026-07-04-glowe-web-translation.md`.
>
> **Landed (live on dev):** AC1–AC6, AC8, AC9 done — `glowe_content_translations` cache (migrations `0219` + `0220` `org_description` + `0221` `org_field`), `glowe-translate` Edge Function reading source rows server-side, `_shared/translation/` provider extraction, `glowe-translate.js` demand-driven translate + "show original" toggle localized via `GLOWE_TRANSLATIONS`. **AC7 partial:** all list/card render paths wired (`renderPostCard`, `renderOpportunityCard`, `renderWishCard`, `renderProjectCard`, `renderOrganizationCard` — incl. the `org_field` category tag) **plus the wish detail modal (`glowe_post` title+text) and the opportunity detail page (`glowe_opportunity` title+description)**; toggle appears only when a translation was applied. **Remaining (TD-135):** the profile detail page (derived prose through `_adaptDbProfile` fallback chains, shared with static profiles) + array/jsonb tags (opportunity requirements/responsibilities/skills, wish areas, profile skills/languages) not yet wired.
