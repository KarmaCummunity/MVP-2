...
| **Last Updated** | 2026-05-10 (Closure UX polish — profile grid auto-refresh after close/reopen via React Query invalidation; post-detail pops back after a successful "סמן כנמסר ✓". `OwnerActionsBar` API split into semantic `onClosed` / `onReopened`.) |
