...
| **Last Updated** | 2026-05-11 (scope-trim — **`EXEC-9`** removes per-user block / unblock from the MVP. `FR-MOD-003`, `FR-MOD-004`, `FR-MOD-009`, `NFR-PRIV-009`, `INV-M1`, `FR-SETTINGS-005` marked `DEPRECATED — post-MVP`; `D-11` superseded. `FR-MOD-010` (false-report sanctions) relocated from the deleted P1.4 row into P1.3. Code surface deleted: `packages/application/src/block/*`, `IBlockRepository`, the `Block` domain entity, `SupabaseBlockRepository`, the `'blocked'` branch in `FollowState`, the `is_blocked()` RPC call in `fetchFollowStateRaw`, the i18n strings, and the `blockUser` / `unblockUser` wiring in `apps/mobile/src/lib/container.ts`. DB schema (`public.blocks`, `is_blocked()` / `has_blocked()`) retained — already in prod, stays unpopulated, returns `false` for all viewers. Follows P1.2.y — realtime rejoin fix; P1.2.x — chat-post anchor lifecycle 🟢 Done across PRs #59 + #60 + #61; P1.9 — close post from chat #57; P1.2 — feed discovery; P1.1.2 — follow-surface cross-platform confirm/notify modals.) |
