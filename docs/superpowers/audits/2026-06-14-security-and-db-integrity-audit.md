...
- **Fix (migration 0199, TD-172):** a `BEFORE UPDATE` guard trigger `posts_guard_client_status_transition` (SECURITY INVOKER, so `current_user` is the real role) enforces the legal status-transition graph for client-originated updates and rejects `reopen_count` rewrites. Trusted SECURITY DEFINER writers run as `postgres` and bypass via `current_user not in ('authenticated','anon')`; the three SECURITY INVOKER closure/reopen RPCs raise a transaction-local `kc.allow_posts_status_write` flag before their UPDATE; the only client-direct transition left is `open → deleted_no_recipient`. Regression test `supabase/tests/0199_posts_status_transition_guard.sql`. (The audit-PR scope below predates this follow-up.)
