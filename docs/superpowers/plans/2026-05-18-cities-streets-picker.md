...
- Create: `supabase/migrations/0098_seed_streets.sql` (output of the script — committed)
...
```javascript
#!/usr/bin/env node
// Generates supabase/migrations/0098_seed_streets.sql from the canonical
...
async function main() {
  ...
  // Emit SQL
  const out = [];
  out.push('-- 0098_seed_streets | Bulk-seed Israeli street list from data.gov.il package 321');
  ...
  const target = path.join(REPO, 'supabase/migrations/0098_seed_streets.sql');
...
Expected: stderr reports `Got 63563 rows`, `Synthesized 9000-sentinel for: 3729=…, 3758=…`, `Wrote 63565 rows to …0098_seed_streets.sql`. File size around 5 MB.

- [ ] **Step 2.1.3: Sanity-check the generated SQL**

```bash
head -20 supabase/migrations/0098_seed_streets.sql && echo '---' && grep -c '^  (' supabase/migrations/0098_seed_streets.sql && echo '---' && grep -E "'5000'" supabase/migrations/0098_seed_streets.sql | head -5
```
...
git add scripts/generate-streets-seed.mjs supabase/migrations/0098_seed_streets.sql
...
(city_id, street_id, name_he), and emits supabase/migrations/0098_seed_streets.sql.
...
**Files:**
- Create: `supabase/migrations/0097_create_streets.sql`
...
```
Run mcp__supabase__apply_migration with name="0097_create_streets" and the SQL body above.
Run mcp__supabase__apply_migration with name="0098_seed_streets" using the body of supabase/migrations/0098_seed_streets.sql.
```
...
psql "$SUPABASE_DB_URL" -f supabase/migrations/0097_create_streets.sql
psql "$SUPABASE_DB_URL" -f supabase/migrations/0098_seed_streets.sql
```
...
```bash
git add supabase/migrations/0097_create_streets.sql app/packages/infrastructure-supabase/src/database.types.ts
...
to regenerate `0098_seed_streets.sql`; the migration is idempotent.
